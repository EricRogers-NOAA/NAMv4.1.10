...
echo "         Jun 2013 - Magee   - Replaced NOGAPS with NAVGEM. "
echo " "
...
#  cmodel -- Model being processed (gfs, mrf, ukmet, ecmwf, nam, ngm, nvgm,
...
export namvitdir=${namvitdir:-${COMROOTp2}/nam/${envir}/nam.$PDY}
...
case ${cmodel} in 
  ...
  gfs) set +x                                       ;
       ...
       echo " "                                     ;
       set -x                                       ;
       gfsdir=${gfsdir:-${COMROOThps}/gfs/prod/gfs.${PDY}}   ;
       gfsgfile=gfs.t${CYL}z.master.grbf            ;
       gfsifile=gfs.t${CYL}z.master.grbif           ;
       COM=${COM:-${COMROOThps}/gfs/${envir}/gfs.${PDY}}     ;
       fcstlen=180                                  ;
  ...
  nam) set +x                                             ;
       ...
       set -x                                             ;
       namdir=$DATA                                       ;
       namgfile=nam.t${CYL}z.awip32tc                     ;
       namifile=nam.t${CYL}z.awip32tci                    ;
       COM=${COMROOTp3}/nam/${envir}/nam.${PDY}             ;
       fcstlen=84
       fcsthrs=' 00 06 12 18 24 30 36 42 48 54 60 66 72 78 
  ...
  nvgm) set +x                                          ;
       ...
       echo " "                                         ;
       set -x                                           ;
       nvgmdir=/com/fnmoc/${envir}/navgem.${PDY}        ;
       nvgmgfile=navgem_${PDY}${CYL}f                   ;
       nvgmifile=                                       ;
       COM=/com/fnmoc/${envir}/navgem.${PDY}            ;
       fcstlen=144                                      ;
       fcsthrs=' 00 12 24 36 48 60 72 84 96 108 120 132 144 
                 99 99 99 99 99 99 99 99 99 99 99 99 99 99  
                 99 99 99 99 99 99 99 99 99 99 99 99 99 99  
  ...
  hdas) set +x                                          ;
       ...
       echo " "                                         ;
       set -x                                           ;
       hdasdir=/com2/gfs/prod/gdas.${PDY}                ;
...
then
  ...
  synvit6ago_dir=/com2/gfs/prod/gfs.${d6ago_4ymd}
  synvit6ago_file=gfs.t${d6ago_hh}z.syndata.tcvitals.tm00
  synvit6ahead_dir=/com2/gfs/prod/gfs.${d6ahead_4ymd}
  synvit6ahead_file=gfs.t${d6ahead_hh}z.syndata.tcvitals.tm00

else
      
#these are for ops on Dell
  synvitdir=${COMROOTp3}/nam/${envir}/nam.${PDY}
  synvitfile=nam.t${CYL}z.syndata.tcvitals.tm00
  synvit6ago_dir=${COMROOTp3}/nam/${envir}/nam.${d6ago_4ymd}
  synvit6ago_file=nam.t${d6ago_hh}z.syndata.tcvitals.tm00
  synvit6ahead_dir=${COMROOTp3}/nam/${envir}/nam.${d6ahead_4ymd}
...
#   vitals such as "91L NAMELESS" or "89E NAMELESS", since NHC 
...
$EXECnam/nam_supvit <${DATA}/suv_input.${atcfout}.${PDY}${CYL}
suvrcc=$?
...
then
  ...
  if [ -s ${DATA}/nvgmgribfile.${PDY}${CYL} ]
  then
    rm ${DATA}/nvgmgribfile.${PDY}${CYL}
  fi
  ...
  do
    ...
    if [ ! -s ${nvgmdir}/${nvgmgfile}${vhour} ]
    then
      set +x
      echo " "
      echo " !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
      echo " !!! NAVGEM File missing: ${nvgmdir}/${nvgmgfile}${vhour}"
      echo " !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
      echo " "
...
#      err_exit " FAILED ${jobid} - MISSING NAVGEM FILE IN TRACKER SCRIPT - ABNORMAL EXIT"
    ...
    do
      case ${parm} in
        "SurfaceU")
          grep "UGRD:10 m " nvgm.ix | grep ${vtstring} | \
            $WGRIB -s $gfile -i -grib -append -o ${DATA}/nvgmgribfile.${PDY}${CYL} ;;
        "SurfaceV")
          grep "VGRD:10 m " nvgm.ix | grep ${vtstring} | \
            $WGRIB -s $gfile -i -grib -append -o ${DATA}/nvgmgribfile.${PDY}${CYL} ;;
                 *)
          grep "${parm}" nvgm.ix | grep ${vtstring} | \
            $WGRIB -s $gfile -i -grib -append -o ${DATA}/nvgmgribfile.${PDY}${CYL} ;;
      esac
    done
  ...
  $gix ${DATA}/nvgmgribfile.${PDY}${CYL} ${DATA}/nvgmixfile.${PDY}${CYL}
  gribfile=${DATA}/nvgmgribfile.${PDY}${CYL}
  ixfile=${DATA}/nvgmixfile.${PDY}${CYL}
      ...
      if [ $cmodel != 'ece' -a $cmodel != 'cens' -a $cmodel != gfs_enr ]; then
        ...
        then
          ...
          do
            ...
            set +x
            echo " "
            echo "+++ Adding records to  TPC ATCFUNIX directory: $ATCFdir/${at}${NO}${syyyy}/ncep_a${at}${NO}${syyyy}"
            echo " "
            set -x
          done
        fi
