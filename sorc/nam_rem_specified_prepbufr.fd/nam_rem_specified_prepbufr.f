...
C  --------------------------------------------------
  ...
  102    FORMAT(/5X,'===> READ IN BUFR DATA MESSAGE NUMBER ',I5,' -- ',
     ...
     $      SUBSET.NE.'SYNDAT  ')  THEN
            IF(SUBSET_LAST.EQ.'ADPSFC  ' .OR.
     $         SUBSET_LAST.EQ.'ADPUPA  ' .OR.
     $         SUBSET_LAST.EQ.'SFCSHP  ' .OR.
     $         SUBSET_LAST.EQ.'SYNDAT  ')  THEN
cppppp
               print *, 'Last message was ',SUBSET_LAST,' CLOSE IT!'
cppppp
               CALL CLOSMG(LUBFJ)
  ...
  103 FORMAT(/5X,'------> COPIED THIS MESSAGE TO OUTPUT FILE -- (ENTRY',
     ...
     $    'ADPSFC  '.OR.SUBSET_LAST.EQ.'SFCSHP  '.OR.SUBSET_LAST.EQ.
     $    'SYNDAT  '.OR.SUBSET_LAST.EQ.'ADPUPA  '))
...
ccc  $       TYP,XOB,YOB,SID
cppppp
!!GP Lou - 210 is SYNDATA, 232 is dropwinsounde, 111 is ps and q
            IF((NINT(TYP).GE.280.AND.NINT(TYP).LE.299) .OR.
     $          NINT(TYP).EQ.210.OR.NINT(TYP).EQ.232   .OR.
     $          NINT(TYP).EQ.111)  THEN
               RLAT=YOB
               RLON=360.-XOB
...
cppppp
            ...
            ELSE
cppppp
ccc            print *, 'DO NOT SKIP, REPORT TYPE IS NOT 280-299 or 210'
cppppp
            END IF
     ...
     $       (SUBSET_LAST.EQ.'ADPSFC  '.OR.SUBSET_LAST.EQ.'SFCSHP  '.OR.
     $        SUBSET_LAST.EQ.'SYNDAT  '))
     $        PRINT 104, SUBSET_LAST,IDATE,IRECO-1
            IRECO_LAST  = IRECO
            SUBSET_LAST = SUBSET
         END DO
...
C  ----------------------------------------------------------------
      ...
      print *, 'COMPLETED'
      print *
      print *, '    - number of ADPUPA,  and/or SYNDAT reports ',
     $ 'skipped = ',ikntskp
      print *
      print *, '    - number of ADPUPA,  and/or SYNDAT reports ',
     $ 'kept = ',ikntkep
