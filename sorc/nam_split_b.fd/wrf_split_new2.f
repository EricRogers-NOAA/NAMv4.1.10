...
!     DECLARE VARIABLES
!
      INTEGER I,J,K,NX,NY,NZ,IBGS,IVOBS,IFLAG_NEST
...
! read data
      ...
      READ(5,*)ITIM,IBGS,IVOBS,IFLAG_NEST
      ...
      ALLOCATE ( PD(NX,NY),ETA1(NZ),ETA2(NZ) )
...
!        IF(Z1(I,J,1).GT.99.and.IBGS.NE.1))THEN
!          US850(I,J)=U1(I,J,KS850)*1.25
!          VS850(I,J)=V1(I,J,KS850)*1.25
!        END IF
      END DO
      END DO
...
!      DO J=1,NY,2
!      DO I=1,NX
!        RLON(I,J)=WBD+(I-1)*2*DX_R
!        RLAT(I,J)=SBD+(J-1)*DY_R
!      END DO
!      END DO
!      DO J=2,NY-1,2
!      DO I=1,NX
!        RLON(I,J)=WBD+(2*I-1)*DX_R
!        RLAT(I,J)=SBD+(J-1)*DY_R
!      END DO
!      END DO

       IF(GLON(1,1).LT.0.)THEN
         DO J=1,NY
         DO I=1,NX
...
! LON & LAT at U,V
        ...
        IF(HLON(1,1).LT.0.)THEN
          DO J=1,NY
          DO I=1,NX
            HLON(I,J)=HLON(I,J)+360.
          END DO
          END DO
        END IF
        IF(VLON(1,1).LT.0.)THEN
          DO J=1,NY
          DO I=1,NX
            VLON(I,J)=VLON(I,J)+360.
          END DO
          END DO
        END IF
...
!C        COMPUTE SEA LEVEL PRESSURE.
      ...
      ENDDO
       ...
       CALL HURR_MESS(ITIM,NX,NY,GLON,GLAT)
...
! PD(I,J)=P1(I,J,1)-PDTOP-PT=PSFC(I,J)-PDTOP-PT
      ...
      DO I=1,NX
        P1(I,J,NZ1)=PT
        DO K=1,NZ
          P1(I,J,NZ1-K)=P1(I,J,NZ1-K+1)+PDTOP*ETA1(K)+PD(I,J)*ETA2(K)     ! PD(I,J) changed
        ENDDO
      ENDDO
      ENDDO
...
!      END DO
 ...
 24       CONTINUE
      ...
      DO K=1,NZ+1
        WRITE(62)((Z1(I,J,K),I=1,NX),J=1,NY)
      END DO
      DO K=1,NZ+1
        WRITE(62)((P1(I,J,K),I=1,NX),J=1,NY)
      END DO
      DO K=1,NZ
        WRITE(62)((T1(I,J,K),I=1,NX),J=1,NY)
      END DO
      DO K=1,NZ
        WRITE(62)((Q1(I,J,K),I=1,NX),J=1,NY)
      END DO
      DO K=1,NZ
        WRITE(62)((U1(I,J,K),I=1,NX),J=1,NY)
      END DO
      DO K=1,NZ
        WRITE(62)((V1(I,J,K),I=1,NX),J=1,NY)
      END DO

      IF(HLON(1,1).GT.180.)THEN
        DO J=1,NY
        DO I=1,NX
          HLON(I,J)=HLON(I,J)-360.
        END DO
...
!        STMNAME(I)='NUL'
      ...
      DO I=1,10
        ...
        STMNAME(I)='NUL'
        READ(30,442,end=436)
     &     (ISTMCY1(J,I),ISTMCX1(J,I),J=1,7,3),STMNAME(I)
        DYM1=0.
        DXM1=0.
        DYP1=0.
        DXP1=0.
        ...
        END IF
        ISTMCY1(2,I)=ISTMCY1(4,I)-DYM1*2.
        ISTMCY1(3,I)=ISTMCY1(4,I)-DYM1*1.
        ISTMCX1(2,I)=ISTMCX1(4,I)-DXM1*2.
        ISTMCX1(3,I)=ISTMCX1(4,I)-DXM1*1.
        ISTMCY1(5,I)=ISTMCY1(4,I)+DYP1*1.
        ISTMCY1(6,I)=ISTMCY1(4,I)+DYP1*2.
        ISTMCX1(5,I)=ISTMCX1(4,I)+DXP1*1
        ISTMCX1(6,I)=ISTMCX1(4,I)+DXP1*2
...
!  Correct to the storm center position
  ...
  430 FORMAT(/' STORM NAME: ',A12,/, ' READIN STORM CENTER=',2F12.4,
     &       /,' STORM DIR and SPEED: ',2F12.4)
      ...
      DO I=1,KSTM
        ...
        IF(ISTMCY1(J,K).EQ.0.and.ISTMCX1(J,K).EQ.0)THEN
            ...
            END DO
            IF(IFWRT.EQ.0)THEN
              XDIST6H=CLON_N(I)-(360.-ISTMCX1(4,K)*0.1)
     ...
     &         GLON(ILO,ILA),GLAT(ILO,ILA),DISTC
      ...
      SLON_N(I) = IFIX(GLON(IC,JC)+0.5 - IRX/2)
      SLAT_N(I) = IFIX(GLAT(IC,JC)+0.5 - JRX/2)
      PRINT *,' '
      PRINT *,'=========================================='
...
!CC  1 FORMAT(/'...FMT=',A10,/,' ...BUFF=',A100)
   ...
   32 FORMAT(A130)
     ...
     &       HLAT,HLON,VLAT,VLON,
     &      lmeta,iswin,iewin,jswin,jewin,IBGS,IVOBS)
!
! ITIM: Time level
...
!  insert storm relocation code here
       ...
       integer    IST,IED,JST,JED,KS850,gd_dim3,IFLAG_NEST
...
!         RDST1=0.06
         ...
         JED=JRX
       END IF
       RDST=max(0.05,2.*ABS(XLON1(2,1)-XLON1(1,1)))
       RDST1=RDST*RDST
...
!       IF(IFLAG_NEST.EQ.1)THEN 
     ...
     &             (XLAT(K,N)-BLAT(J))**2
           IF(DIST.LE.RDST1)THEN
             if(NGRID .ge. gd_dim3) then
                print*,'Error!  In wrf_split_new2.f, NGRID > gd_dim3'
                print*,'Make gd_dim3 larger.  Currently, it is: ',      &
     &                 gd_dim3
                stop 12
             end if
             NGRID=NGRID+1
...
!.. DETERMINE FILTER DOMAIN D0 (=1.25*Rf)
      ...
      CALL FILTER(RS,TW,RF,RFAVG,KST,IBGS,IVOBS)
...
!        IW = ING(I)
!        JW = JNG(I)
!        DKM(IW,JW)=DKY(IW,JW)-(DKY(IW,JW)-DKM(IW,JW))*RRIJ(I)
!      END DO
...
!c      ENDDO
      ...
      SUBROUTINE FILTER(RS,TW,RF,RFAVG,KST,IBGS,IVOBS)
