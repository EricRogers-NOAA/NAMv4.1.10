...
!$$$ end documentation block
 ...
 !use hybrid_ensemble_parameters, only: add_bias_perturbation
  use ensemble_fastread_commvar, only: ens_fast_read
  ...
  if(ens_fast_read==-1) call ensemble_preread   ! may use more total memory if called here
                                                ! compare with option to call ensemble_preread
                                                ! in gsi_mod (??) if ens_fast_read = 1  
...
!    allocate necessary space on global grid
     ...
     if(use_gfs_nemsio)then
        if(ens_fast_read /= 0) then
           call move2bundle_(n,it,atm_bundle,iret)
        else
