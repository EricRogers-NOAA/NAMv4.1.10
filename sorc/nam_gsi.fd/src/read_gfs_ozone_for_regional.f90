...
!$$$ end documentation block
  ...
  use gridmod, only: wrf_nmm_regional,use_gfs_nemsio
  use constants,only: zero,half,fv,rd_over_cp,one,h300,rad2deg
  use constants, only: max_varname_length,i_missing,r60,r3600
  ...
  use sigio_module, only: sigio_intkind,sigio_head,sigio_srhead,sigio_sclose,sigio_sropen
  ...
  integer(i_kind) inner_vars,num_fields,num_fieldst,nlat_gfs,nlon_gfs,nsig_gfs,jcap_gfs,jcap_gfs_test
  integer(i_kind) nord_g2r,jcap_org,nlon_b

   integer(i_kind),dimension(7):: idate
   integer(i_kind) :: nfhour, nfminute, nfsecondn, nfsecondd
   integer(i_kind) :: idvc,idsl,lonb,latb,levs,nvcoord
...
! Extract header information
           ...
           idsl=sighead%idsl
           idvc=sighead%idvc
           nlat_gfs=sighead%latb+2
           nlon_gfs=sighead%lonb
        ...
        allocate(vcoord(nsig_gfs+1,3,2))
         vcoord(1:nsig_gfs+1,1:sighead%nvcoord,1)=sighead%vcoord(1:nsig_gfs+1,1:sighead%nvcoord)
          ...
          call nemsio_getfilehead(gfile_atm, idate=idate, gtype=filetype,  &
           ...
           hourg = float(nfhour) + float(nfminute)/r60 + float(nfsecondn)/float(nfsecondd)/r3600
...
!       nvcoord is no longer part of NEMSIO header output.
        ...
        if(maxval(vcoord(:,3,1))==zero .and. &
          ...
          if(mype == 0) then
             ...
             write(6,*) ' iadate(y,m,d,hr,min)=',iadate
             write(6,*) ' jcap,levs=',jcap_gfs,levs
